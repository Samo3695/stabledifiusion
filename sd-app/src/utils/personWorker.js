/**
 * Web Worker pre výpočet pohybu postáv
...
self.onmessage = function(e) {
  ...
  switch (type) {
    ...
    case 'findNextMove':
      // Vypočítaj ďalší pohyb pre osobu
    ...
    case 'batchFindNextMove':
      // Vypočítaj ďalší pohyb pre viacero osôb naraz
      const moves = data.persons.map(person => ({
        personId: person.id,
        target: findNextMove(person.currentCell)
      }))
      self.postMessage({
        type: 'batchNextMoves',
        data: moves
...
/**
 * Nájde ďalší tile kam sa má osoba presunúť
 */
